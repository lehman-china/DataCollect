...
public class LoginSCMobile {
    ...
    public static void main(String args[]) throws Exception {
        ...
        Matcher matcher = Pattern.compile("var keyStr(.*?)function enCode").matcher(loginString.replaceAll("\\r|\\t|\\n|\\a", ""));

//        使用java截取js方法,首先,将换行符制表符回车符报警符都替换掉,这样在截取时就不会出问题了
//        Matcher matcher = Pattern.compile("initdata=\\{(.*?)\\}</script>").matcher(loginString.replaceAll("\\r|\\t|\\n|\\a",""));
        ...
        while (matcher.find()) {
            ...
            Object encrypt_password = invocable.invokeFunction("encode64", password);
            //第二次加密
            String secondeEncryptURL = "http://www.sc.10086.cn/ssoLogin.do?dispatch=epwd&pwd=" + encrypt_password;
            ...
            Tools.saveToLocal(httpClient.execute(imageLoginGet).getEntity(), System.currentTimeMillis() + "imageCode.png");
            ...
            parameters.add(new BasicNameValuePair("user_passwd", encrypt_password.toString()));
            ...
            //登录结果
            String loginResult = EntityUtils.toString(LoginPostResponse.getEntity());
            if (!loginResult.contains("验证码不正确") && !loginResult.contains("密码有误")) {
                //每月账单
                String month = "201405";
                String date_end = "30";
                String monthBillURL = "http://www.sc.10086.cn/historybill.do?dispatch=getHistoryBill&stime=" + month;
                HttpPost monthBillPost = new HttpPost(monthBillURL);
                Tools.saveToLocal(httpClient.execute(monthBillPost).getEntity(), "CHINAMOBILESC.bill." + month + ".html");

                //详单
                //1) 发送短信验证码
                String sendSMSURL = "http://www.sc.10086.cn/bill.do";
                List<NameValuePair> smsParameters = new ArrayList<NameValuePair>();
                smsParameters.add(new BasicNameValuePair("dispatch", "getBillRandomPwd"));
                ...
                sendSmsHttpPost.setEntity(new UrlEncodedFormEntity(smsParameters));
                String sendResult = EntityUtils.toString(httpClient.execute(sendSmsHttpPost).getEntity());
                if (sendResult.length() < 10) {
                    //2)验证短信验证码
                    String smsCode = JOptionPane.showInputDialog("请输入短信验证码:");
                    String checkSMSURL = "http://www.sc.10086.cn/bill.do?dispatch=checkBillByType&Action=get&ctype=2&month_begin=" + month + "&date_begin=1&date_end=" + date_end + "&call_type=0&sec_pwd=" + smsCode;
                    HttpGet checkSmsCodeGet = new HttpGet(checkSMSURL);
                    String checkSmsCodeResult = EntityUtils.toString(httpClient.execute(checkSmsCodeGet).getEntity());
                    if (checkSmsCodeResult.contains("000")) {
                        //3)获取数据
                        String detailURL = "http://www.sc.10086.cn/bill.do?dispatch=getBillByType&ctype=2&month_begin=" + month + "&date_begin=1&date_end=" + date_end + "&call_type=0&sec_pwd=" + smsCode;
                        HttpGet detailHTTPGET = new HttpGet(detailURL);
                        detailHTTPGET.getParams().setParameter("Content-Type","application/x-www-form-urlencoded");
                        Tools.saveToLocal(httpClient.execute(detailHTTPGET).getEntity(), "detail." + month + ".html");
                    }
                }
